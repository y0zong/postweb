...
>(({ className, ...props }, ref) => (
  <Primitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-window bg-black bg-opacity-70 backdrop-blur-sm transition-all duration-100 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=open]:fade-in",
      className
    )}
    {...props}
  />
...
>(({ className, close, children, ...props }, ref) => (
  <Portal>
    ...
    <Primitive.Content
      ref={ref}
      className={cn(
        "émodal fixed overflow-hidden rounded bg-window left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] max-h-[calc(100%+1rem)] py-2 z-window flex flex-col animate-raise-with-scale",
        className
      )}
      {...props}
    >
...
const Header = ({
  ...
  <div
    className={cn(
      "flex items-center px-6 pt-6 pb-4 rounded-t mt-[calc(1px-.5rem)] space-x-1.5 bg-window-header border-b mx-[1px] border-b-modal-border",
      className
    )}
    {...props}
  />
...
const Footer = ({
  ...
  <div className={cn("flex items-center justify-end space-x-4 px-6 pb-6 pt-4 rounded-b mb-[-.5rem] bg-window-header ring ring-txt ring-opacity-15 ring-inset ring-offset-[-2px]", className)}{...props} />
)
