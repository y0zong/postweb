    ...
    return <Dialog.Root open={open} onOpenChange={setOpen}>
        ...
        <Dialog.Content forceMount close={isContent}>
            ...
            {(ensure || cancel) && <Dialog.Footer>
                {cancel && <Dialog.Close asChild><Button variant="secondary">{cancel}</Button></Dialog.Close>}
                <Button intent="primary" form={formId} type="submit" className="w-20">{ensure}</Button>
            </Dialog.Footer>}
        </Dialog.Content>
    </Dialog.Root>
}
